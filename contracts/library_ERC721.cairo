...
# Based on OpenZeppelin Cairo Contracts v0.1.0 (token/erc721/library.cairo) with some tweaks to get proxied caller
...
@storage_var
func ERC721_account_contract_()->(address:felt):
end
...
func ERC721_initializer{
    ...
    ERC721_account_contract_.write(account)
    return ()
end
...
func ERC721_account_contract{
        syscall_ptr : felt*,
        pedersen_ptr : HashBuiltin*,
        range_check_ptr
    }() -> (account: felt):
    let (account_contract) = ERC721_account_contract_.read()
...
func ERC721_transferFrom{
    ...
    # the following tweak is necessary to ensure that calls to the ERC721 portion of the Account that get routed through the
    # __execute__ function work correctly
    # if Account has indeed called this function that means the owner of the Account has called and hence we need to treat
    # the owner as the actual caller for the purposes of this function
    # similar tweaks have to be done to all other external facing functions of ERC721
    if caller == account_contract:
        let token_0: Uint256 = Uint256(0,0)
        let (local new_temp_caller) = ERC721_ownerOf(token_0)
        let (is_approved) = _is_approved_or_owner(new_temp_caller, token_id)
    ...
    let (is_approved_caller) = _is_approved_or_owner(caller, token_id)
    with_attr error_message("ERC721: either is not approved or the caller is the zero address"):
        assert_not_zero(caller * is_approved_caller)
    end
